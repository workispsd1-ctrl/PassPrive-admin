...
const TABLE = "store_mood_categories";
...
export async function GET() {
  try {
    ...
    const message = err instanceof Error ? err.message : "Failed to load store mood categories";
  ...
  try {
    ...
    if (!hasLightImage && !hasDarkImage) {
      return NextResponse.json(
        { error: "At least one image is required to create a store mood category" },
        { status: 400 }
      );
    }
    ...
    const { data, error } = await supabaseAdmin.from(TABLE).insert(record).select().maybeSingle();
    if (error) throw error;
    return NextResponse.json(data, { status: 201 });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Failed to create store mood category";
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
