...
export async function GET() {
  try {
    ...
    const message = err instanceof Error ? err.message : "Failed to load mood categories";
  ...
  try {
    ...
    if (!hasLightImage && !hasDarkImage) {
      return NextResponse.json(
        { error: "At least one image is required to create a mood category" },
        { status: 400 }
      );
    }
    ...
    const message = err instanceof Error ? err.message : "Failed to create mood category";
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
