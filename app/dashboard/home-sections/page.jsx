...
// Superadmin CRUD for the app's Home section ordering / titles / templates.
// Backed by the `session_sorting` (screens) + `session_sorting_titles` tables —
// the same rows the mobile app reads to lay out Home. See SECTION_CMS.md.
export default function HomeSectionsAdminPage() {
  const [showForm, setShowForm] = useState(false);
  const [editingItem, setEditingItem] = useState(null);
  const [screen, setScreen] = useState(null); // { id, name }
          ...
          >
            <CardHeader className="space-y-4 border-b border-slate-100/90 bg-white/70 px-4 py-4 sm:px-5">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  <CardTitle className="text-[18px] leading-6 text-slate-900">Home Sections</CardTitle>
                  <CardDescription className="mt-1 text-[12px] leading-5 text-slate-500">
                    {showForm
                      ? "Set the heading, type (native or template), order and style. Save to publish."
                      : "Reorder, rename, show/hide and add sections on the app's Home screen."}
                  </CardDescription>
                </div>
                ...
                {!showForm ? (
                  <button
                    ...
                    disabled={!screen}
                    className="inline-flex h-10 items-center gap-2 rounded-2xl bg-[#5800AB] px-5 text-sm font-semibold text-white shadow-[0_10px_20px_rgba(88,0,171,0.25)] hover:bg-[#4a0090] disabled:opacity-50"
                  >
